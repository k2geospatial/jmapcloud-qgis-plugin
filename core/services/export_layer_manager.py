...
from qgis.core import Qgis, QgsApplication, QgsFeedback, QgsProject, QgsMessageLog
...
class ExportLayerManager(QObject):
    data_source_references_loaded = pyqtSignal(list) # Emits the list of project references that are referencing the datasource, each reference contains the project name and the list of layer names referencing the datasource
    ...
    def create_new_layer(self, organisation_id: str, export_selected_layer_data: ExportSelectedLayerData):
        self._start_layer_exportation(organisation_id, export_selected_layer_data)
       
    def replace_layer(self, organisation_id: str, export_selected_layer_data: ExportSelectedLayerData):
        self._start_layer_exportation(organisation_id, export_selected_layer_data)

    
    def _start_layer_exportation(self, organisation_id: str, export_selected_layer_data: ExportSelectedLayerData):
    ...
    def _convert_layer_to_zip(self, organisation_id: str, export_selected_layer_data: ExportSelectedLayerData):
        ...
        convert_layer_to_zip_task.error_occurred.connect(lambda error_message: self._errors.append(error_message))
    ...
    def _upload_layer_files(self, layer_data: LayerData, layer_file: LayerFile, organisation_id: str, export_selected_layer_data: ExportSelectedLayerData):
        ...
        files_upload_manager = FilesUploadManager(self._request_manager, [layer_data], [layer_file], organisation_id)

        def next_step(layers_data):
            layer_data_result = layers_data[0]

            if self._validate_layer_errors(layer_data_result, self.tr("Upload layer files")) is None:
                self._finish(False)
                return
            
            if export_selected_layer_data.mode == ExportSelectedLayerData.ExportMode.create:
                self._create_datasource(layer_data_result, organisation_id, export_selected_layer_data)
            else:
                self._update_datasource(layer_data_result, organisation_id, export_selected_layer_data)
    ...
    def _create_datasource(self, layer_data: LayerData, organisation_id: str, export_selected_layer_data: ExportSelectedLayerData):
        if self._is_canceled:
            return
        ...
        datasource_manager = DatasourceManager(self._request_manager, [layer_data], organisation_id, export_selected_layer_data.mode)
        ...
        def next_step(layers_data):
            ...
            self._add_layer_to_jmc_project(layer_data_result, organisation_id, export_selected_layer_data)
    ...
    def _update_datasource(self, layer_data: LayerData, organisation_id: str, export_selected_layer_data: ExportSelectedLayerData):
        if self._is_canceled:
            return
        ...
        def next_step(layers_data):
            ...
            self._replace_layer_in_jmc_project(layer_data_result, organisation_id, export_selected_layer_data)
    ...
    def _add_layer_to_jmc_project(
        ...
        create_layer_task = CreateLayerTask(self._jmap_mcs, organisation_id, layer_data, export_selected_layer_data.JMC_project)
        ...
        create_layer_task.error_occurred.connect(lambda error_message: self._errors.append(error_message))
    ...
    def _replace_layer_in_jmc_project(self, layer_data: LayerData, organisation_id: str, export_selected_layer_data: ExportSelectedLayerData):
        if self._is_canceled:
            return
        ...
        # replace layer task
        replace_layer_task = ReplaceLayerTask(self._jmap_mcs, organisation_id, layer_data, export_selected_layer_data)
    ...
    def _export_style(self, layer_data: LayerData, export_selected_layer_data: ExportSelectedLayerData, should_remove_old_styles: bool = False):
        ...
        export_layer_style_task.error_occurred.connect(lambda error_message: self._errors.append(error_message))
    ...
    def _validate_layer_errors(self, layer_data: LayerData | None, step_string: str) -> LayerData | None:
        ...
        if layer_data.layer_file is not None and layer_data.layer_file.upload_status != LayerFile.Status.no_error:
