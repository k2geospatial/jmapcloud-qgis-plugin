...
class LoadJMCDataSourceReferencesTask(CustomQgsTask):
    # Emits the list of project references that are referencing
    # the datasource, each reference contains the project name and the list of layer names
    # referencing the datasource
    ...
    def run(self):
        ...
        reply: RequestManager.ResponseData = self._jmap_mcs.get_datasource_references_by_id(
        ...
        project_references: dict[str, list[str]] = reply.content["additionalInfo"]["references"][
            "projects"
        ]
        ...
        project_references.pop(
            self._project_id, None
        )  # Remove the current project from the references

        result: list[dict[str, list[str]]] = (
            []
        )  # List of dict with project name and list of layer names referencing the datasource
        ...
        for project_id, layers in project_references.items():
            ...
            project_name = project_data.content["name"][
                locale if locale in project_data.content["name"] else "en"
            ]
            ...
            for layer_id in layer_ids:
                ...
                if not layer_data:
                    self.error_occur(
                        self.tr(
                            "Error loading layer details for layer id {} in project id {}"
                        ).format(layer_id, project_id),
                        MESSAGE_CATEGORY,
                    )
                    continue
                ...
                if layer_data.status != QNetworkReply.NetworkError.NoError:
                    self.error_occur(
                        self.tr(
                            "Error loading layer details for layer id {} in project id {}: {}"
                        ).format(layer_id, project_id, layer_data.error_message),
                        MESSAGE_CATEGORY,
                    )
                    continue

                layer_name = layer_data.content["name"][
                    locale if locale in layer_data.content["name"] else "en"
                ]
