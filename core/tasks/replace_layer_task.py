...
from ..views import LayerData, ExportSelectedLayerData
...
from ..plugin_util import convert_scale_to_zoom, convert_QGIS_text_expression_to_JMap
...
class ReplaceLayerTask(CustomQgsTask):
    ...
    def __init__(self, jmap_mcs: JMapMCS, organisation_id: str,  layer_data: LayerData, export_selected_layer_data: ExportSelectedLayerData):
        super().__init__(f"Replacing layer {layer_data.layer_name} in JMC project {export_selected_layer_data.JMC_project.name}", CustomQgsTask.CanCancel)
    ...
    def _replace_layer_in_jmc_project(self):
        ...
        if not request_payload:
            self.error_occur(self.tr("Error creating request to replace layer in JMC"), MESSAGE_CATEGORY)
        ...
        if reply.status != QNetworkReply.NetworkError.NoError:
            self.error_occur(self.tr("Error replacing layer in JMC: {}").format(reply.error_message), MESSAGE_CATEGORY)
    ...
    def _create_layer_patch_request(self, layer_data: LayerData) -> UpdateLayerDTO | None:
        ...
        layer_dto.minimumZoom = convert_scale_to_zoom(qgis_layer.minimumScale()) if qgis_layer.minimumScale() != 0 else None
        layer_dto.maximumZoom = convert_scale_to_zoom(qgis_layer.maximumScale()) if qgis_layer.maximumScale() != 0 else None
        ...
        if layer_data.layer_type in [LayerData.LayerType.file_vector, LayerData.LayerType.API_FEATURES]:
          ...
          if qgis_layer.labelsEnabled():
              ...
              labeling_config_dto = LabelingConfigDTO.from_qgs_labeling(labeling, default_language)
              if not labeling_config_dto:
                  self.error_occur(self.tr(
                        "Error creating labeling for layer {}, JMap Cloud only support single rule labeling"
                    ).format(layer_data.layer_name), MESSAGE_CATEGORY)
              else:
        ...
        layer_dto.mouseOverConfiguration = MouseOverConfigDTO(qgis_layer.mapTipsEnabled(), mouse_over_text)
        return layer_dto
