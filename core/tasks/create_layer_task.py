...
class CreateLayerTask(CustomQgsTask):
  ...
  def __init__(self, jmap_mcs: JMapMCS, organisation_id: str, layer_data: LayerData, JMC_project: ProjectData):
       super().__init__(f"Creating layer {layer_data.layer_name} in JMC project {JMC_project.name}", CustomQgsTask.CanCancel)
  ...
  def _create_layer_in_jmc(self):
      ...
      if not request:
          self.error_occur(self.tr("Error creating request to create layer in JMC"), MESSAGE_CATEGORY)
          self.layer_creation_finished.emit(None)
          return False
      
      reply = self._jmap_mcs.post_layer(self._organisation_id, self._JMC_project.project_id, request)

      if reply.status != QNetworkReply.NetworkError.NoError:
          self.error_occur(self.tr("Error creating layer in JMC: {}").format(reply.error_message), MESSAGE_CATEGORY)
  ...
  def _create_layer_post_request(self, layer_data: LayerData) -> LayerDTO | None:
        ...
        layer_dto = LayerDTO(
            layer_data.datasource_id, {self._JMC_project.default_language: layer_data.layer_name}, dto_type
        )
        ...
        if layer_data.layer_type in [LayerData.LayerType.API_FEATURES, LayerData.LayerType.file_vector]:
            ...
            if layer.labelsEnabled():
                ...
                labeling_dto = LabelingConfigDTO.from_qgs_labeling(labeling, self._JMC_project.default_language)
